...
export default function BidDetail({ bid: initialBid, user, onBack, defaultSection = 'pricing' }) {
  ...
  async function exportToPdf() {
    const element = printRef.current || document.getElementById('bid-printable-view');
    if (!element) {
      setToast('Export target not available yet.');
      return;
    }

    setExportingPdf(true);
    const filename = `Bid-${bid.id}-Approved.pdf`;
    const opts = {
    ...
    try {
      ...
      setToast(`PDF saved as ${filename}`);
    } catch (err) {
      setToast(`PDF export failed: ${err.message || err}`);
    } finally {
      // Small delay to ensure state doesn't flicker
  ...
  return (
    <div>
      ...
      <div className="detail-header">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            {allApproved && (
              <button className="btn btn-primary" disabled={busy || exportingPdf} onClick={exportToPdf}>
                {Icon.doc(12)} {exportingPdf ? 'Generating PDF…' : 'Export PDF'}
              </button>
            )}
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, height: 0, width: 0, overflow: 'hidden' }}>
        <div id="bid-printable-view" ref={printRef} style={{ 
          width: '208mm', 
          background: '#ffffff', 
          color: '#000000',
          padding: '0'
        }}>
          <PrintableView bid={bid} sections={sections} risks={risks} pricingLines={pricingLines} logoSrc={logoSrc} />
        </div>
      </div>
...
function PrintableView({ bid, sections, risks, pricingLines, logoSrc }) {
  if (!bid || !sections) return <div style={{ padding: 50 }}>Missing data for PDF generation.</div>;

  const totalCost = (pricingLines || []).reduce((sum, l) => sum + (l.rate || 0) * (l.hours || 0), 0);
  const totalBid = (pricingLines || []).reduce((sum, l) => {
    const isML = l.task?.includes('Pipeline Build — Transformation') || l.task?.includes('ML');
    const useM = isML ? bid.currentMargin || bid.margin : l.margin;
    return sum + (l.rate || 0) * (l.hours || 0) * (1 + (useM || 0) / 100);
  }, 0);

  const highRisks = (risks || []).filter(r => r.severity === 'High');

  const SectionTitle = ({ title, sub, noBreak }) => (
    <div style={{ 
      borderBottom: '1.5pt solid #5929d0', 
      paddingBottom: '6pt', 
      marginBottom: '14pt', 
      marginTop: noBreak ? '10pt' : '32pt', 
      pageBreakBefore: noBreak ? 'avoid' : 'always', 
      display: 'block',
      clear: 'both'
    }}>
      <div style={{ fontSize: '18pt', fontWeight: 700, color: '#0F172A', lineHeight: 1.2 }}>{title}</div>
      {sub && <div style={{ fontSize: '9.5pt', color: '#64748B', marginTop: '3pt', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>{sub}</div>}
    </div>
  );

  const PrintTable = ({ headers, children }) => (
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '9.5pt', marginBottom: '20pt', tableLayout: 'auto' }}>
      <thead>
      ...
      <tbody style={{ pageBreakInside: 'auto' }}>{children}</tbody>
    </table>
  );

  return (
    <div style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif', color: '#111', width: '100%', background: '#fff' }}>
      {/* --- COVER PAGE --- */}
      <div style={{ pageBreakAfter: 'always', padding: '40mm 20mm', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: 'linear-gradient(135deg, #ffffff 0%, #f1f5f9 100%)', position: 'relative', boxSizing: 'border-box' }}>
        <div style={{ zIndex: 1 }}>
          <img src={logoSrc} alt="Logo" style={{ height: '42pt', marginBottom: '40pt' }} />
          <div style={{ fontSize: '12pt', letterSpacing: '0.3em', textTransform: 'uppercase', color: '#5929d0', fontWeight: 700, marginBottom: '20pt' }}>Bid Proposal</div>
          <div style={{ fontSize: '42pt', fontWeight: 800, lineHeight: 1, color: '#0F172A', marginBottom: '24pt' }}>{bid.title}</div>
          <div style={{ width: '80pt', height: '6pt', background: '#5929d0', marginBottom: '36pt', borderRadius: '3pt' }} />
          <div style={{ fontSize: '15pt', color: '#475569', maxWidth: '480pt', lineHeight: 1.6 }}>
            Comprehensive delivery plan and commercial framework prepared specifically for <strong>{bid.client}</strong>.
          </div>
        </div>

        <div style={{ zIndex: 1 }}>
          <div style={{ display: 'flex', gap: '48pt', marginBottom: '48pt' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '10pt', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '10pt', fontWeight: 600 }}>Client Partner</div>
              <div style={{ fontSize: '20pt', fontWeight: 700, color: '#0F172A' }}>{bid.client}</div>
              <div style={{ fontSize: '12pt', color: '#64748B', marginTop: '6pt' }}>Ref: {bid.rfp}</div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '10pt', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '10pt', fontWeight: 600 }}>Proposal Details</div>
              <div style={{ fontSize: '20pt', fontWeight: 700, color: '#0F172A' }}>Bid {bid.id}</div>
              <div style={{ fontSize: '12pt', color: '#64748B', marginTop: '6pt' }}>Issued: {new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}</div>
            </div>
          </div>
          
          <div style={{ borderTop: '1px solid #E2E8F0', paddingTop: '24pt', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '11pt', color: '#64748B', fontWeight: 600 }}>
              Submission Deadline: {new Date(bid.deadline).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
            </div>
            <div style={{ fontSize: '10pt', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.2em' }}>
      ...
      <div style={{ pageBreakAfter: 'always', padding: '20mm', boxSizing: 'border-box' }}>
        <div style={{ fontSize: '24pt', fontWeight: 800, color: '#0F172A', marginBottom: '40pt' }}>Contents & Overview</div>
        
        <div style={{ display: 'flex', gap: '40pt' }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '12pt', fontWeight: 700, color: '#5929d0', marginBottom: '20pt', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Table of Contents</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12pt' }}>
              {['Pricing & Commercials', 'Effort Estimation', 'Delivery Schedule', 'Contracted Deliverables', 'Acceptance Criteria', 'Quality Framework', 'Dependency Map', 'Risk Register'].map((item, idx) => (
                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px dotted #E2E8F0', paddingBottom: '6pt' }}>
                  <div style={{ fontSize: '11pt', color: '#334155', fontWeight: 500 }}>{idx + 1}. {item}</div>
                  <div style={{ fontSize: '10pt', color: '#94A3B8' }}>Page {idx + 3}</div>
                </div>
              ))}
            </div>
          </div>

          <div style={{ flex: 1, background: '#F8FAFC', borderRadius: '16pt', padding: '24pt', border: '1px solid #E2E8F0' }}>
            <div style={{ fontSize: '12pt', fontWeight: 700, color: '#0F172A', marginBottom: '16pt' }}>Executive Summary</div>
            <div style={{ fontSize: '10.5pt', color: '#64748B', lineHeight: 1.6, marginBottom: '20pt' }}>
              This proposal outlines the technical and commercial approach for the <strong>{bid.title}</strong> engagement. 
              Our delivery framework is designed to ensure maximum quality and risk mitigation while maintaining transparency across all task levels.
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12pt' }}>
              {[
                { label: 'Proposed Value', val: fmtMoney(bid.value, bid.currency) },
                { label: 'Win Prob.', val: `${Math.round(bid.winProb * 100)}%` },
                { label: 'Total Effort', val: `${pricingLines.reduce((s,l)=>s+l.hours,0)}h` },
                { label: 'Risk Profile', val: highRisks.length > 0 ? 'High' : 'Standard' }
              ].map((kpi, i) => (
                <div key={i} style={{ padding: '10pt', background: '#fff', borderRadius: '8pt', border: '1px solid #E2E8F0' }}>
                  <div style={{ fontSize: '8pt', color: '#94A3B8', textTransform: 'uppercase', fontWeight: 700, marginBottom: '4pt' }}>{kpi.label}</div>
                  <div style={{ fontSize: '12pt', fontWeight: 700, color: '#0F172A' }}>{kpi.val}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ marginTop: '40pt' }}>
          <div style={{ fontSize: '12pt', fontWeight: 700, color: '#0F172A', marginBottom: '16pt' }}>Compliance & Approval Status</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8pt' }}>
            {sections.map(s => (
              <div key={s.section_key} style={{ fontSize: '9pt', padding: '5pt 10pt', borderRadius: '6pt', background: s.status === 'approved' ? '#DCFCE7' : '#F1F5F9', color: s.status === 'approved' ? '#166534' : '#64748B', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6pt', border: `1px solid ${s.status === 'approved' ? '#BBF7D0' : '#E2E8F0'}` }}>
                {s.status === 'approved' && Icon.check(10)} {s.section_name}
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding: '0 20mm' }}>
        {/* --- 1. PRICING SECTION --- */}
        <SectionTitle title="1. Commercial Framework & Pricing" sub={`Rate card applied · ${bid.currency} · ${pricingLines.length} line items`} noBreak />
      <PrintTable headers={[
        { label: 'Task / Activity' },
        { label: 'Role' },
        ...
        {pricingLines.map((l, i) => {
          ...
          return (
            <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
              <td style={{ padding: '8pt', fontWeight: 600 }}>{l.task}</td>
              <td style={{ padding: '8pt' }}>{l.role}</td>
              <td style={{ padding: '8pt', textAlign: 'right' }}>£{l.rate.toLocaleString()}</td>
              <td style={{ padding: '8pt', textAlign: 'right' }}>{l.hours}</td>
              <td style={{ padding: '8pt', textAlign: 'right' }}>{useM}%</td>
              <td style={{ padding: '8pt', textAlign: 'right', fontWeight: 700 }}>£{Math.round(subtotal).toLocaleString()}</td>
            </tr>
          );
        })}
        <tr style={{ background: '#F8FAFC', fontWeight: 700 }}>
          <td colSpan="5" style={{ padding: '10pt 8pt', textAlign: 'right' }}>Total Bid Value</td>
          <td style={{ padding: '10pt 8pt', textAlign: 'right', color: '#5929d0', fontSize: '11pt' }}>{fmtMoney(totalBid, bid.currency)}</td>
        </tr>
      </PrintTable>

      {/* --- 2. EFFORT ESTIMATION --- */}
      <SectionTitle title="2. Effort Estimation" sub="Detailed task breakdown by role and hours" />
      {(() => {
        const effortSec = sections.find(s => s.section_key === 'effort');
        const data = effortSec?.compiled_data?.tasks || [];
        if (!data.length) return <em style={{ fontSize: '10pt', color: '#94A3B8' }}>No data available.</em>;
        return (
          <PrintTable headers={[
            { label: 'ID' },
            { label: 'Task Name' },
            ...
            {data.map((r, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                <td style={{ padding: '8pt', fontWeight: 700 }}>{r.id}</td>
                <td style={{ padding: '8pt' }}>{r.name}</td>
                <td style={{ padding: '8pt' }}>{r.role}</td>
                <td style={{ padding: '8pt', textAlign: 'right' }}>{r.hours}h</td>
                <td style={{ padding: '8pt', textAlign: 'right' }}>
                  <span style={{ color: r.confidence < 0.65 ? '#DC2626' : '#16A34A', fontWeight: 600 }}>{(r.confidence * 100).toFixed(0)}%</span>
                </td>
              </tr>
            ))}
          </PrintTable>
      ...
      {/* --- 3. DELIVERY SCHEDULE --- */}
      <SectionTitle title="3. Delivery Schedule & Milestones" sub="Project timeline and critical delivery points" />
      {(() => {
        const schedSec = sections.find(s => s.section_key === 'schedule');
        const phases = schedSec?.compiled_data?.phases || [];
        const milestones = schedSec?.compiled_data?.milestones || [];
        const totalWeeks = schedSec?.compiled_data?.duration_weeks || 14;
        
        return (
          <div style={{ pageBreakInside: 'avoid' }}>
            {/* Gantt Visual in Print */}
            <div style={{ marginBottom: '20pt', padding: '16pt', background: '#F8FAFC', borderRadius: '8pt', border: '1px solid #E2E8F0' }}>
              <div style={{ fontSize: '7.5pt' }}>
                <div style={{ display: 'flex', marginBottom: '6pt' }}>
                  <div style={{ width: '80pt' }} />
                  {Array.from({ length: totalWeeks }, (_, i) => <div key={i} style={{ flex: 1, textAlign: 'center', color: '#94A3B8', fontWeight: 600 }}>W{i + 1}</div>)}
                </div>
                {phases.map((p, pi) => (
                  <div key={pi} style={{ display: 'flex', alignItems: 'center', marginBottom: '4pt' }}>
                    <div style={{ width: '80pt', fontWeight: 700, fontSize: '9pt', color: '#334155', paddingRight: '8pt' }}>{p.name}</div>
                    {Array.from({ length: totalWeeks }, (_, i) => (
                      <div key={i} style={{ flex: 1, height: '10pt', background: p.weeks.includes(i + 1) ? '#5929d0' : '#E2E8F0', borderRadius: '2pt', margin: '0 1.5pt' }} />
                    ))}
                  </div>
                ))}
              </div>
            ...
            <PrintTable headers={[
              ...
              {milestones.map((m, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  <td style={{ padding: '8pt', fontWeight: 700 }}>{m.name}</td>
                  <td style={{ padding: '8pt' }}>{m.start_date}</td>
                  <td style={{ padding: '8pt' }}>{m.end_date}</td>
                  <td style={{ padding: '8pt' }}>{m.predecessor || '—'}</td>
                  <td style={{ padding: '8pt' }}><span style={{ fontSize: '8pt', padding: '2pt 6pt', background: '#F1F5F9', borderRadius: '4pt', fontWeight: 600 }}>{m.mode}</span></td>
                </tr>
              ))}
            </PrintTable>
          </div>
      ...
      {/* --- 4. DELIVERABLES --- */}
      <SectionTitle title="4. Contracted Deliverables" sub="Defined outputs and responsible parties" />
      {(() => {
        const delSec = sections.find(s => s.section_key === 'deliverables');
        const data = delSec?.compiled_data?.items || delSec?.compiled_data || [];
        if (!data.length) return <em style={{ fontSize: '10pt', color: '#94A3B8' }}>No data available.</em>;
        return (
          <PrintTable headers={[
            { label: 'ID' },
            ...
            {data.map((d, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                <td style={{ padding: '8pt', fontWeight: 700, width: '40pt' }}>{d.id}</td>
                <td style={{ padding: '8pt' }}>
                  <div style={{ fontWeight: 600, fontSize: '10pt' }}>{d.name}</div>
                  <div style={{ fontSize: '8.5pt', color: '#64748B', marginTop: '2pt' }}>{d.description}</div>
                </td>
                <td style={{ padding: '8pt', width: '80pt' }}>{d.format}</td>
                <td style={{ padding: '8pt', width: '100pt' }}>{d.responsible}</td>
              </tr>
            ))}
          </PrintTable>
        );
      ...
      <SectionTitle title="5. Acceptance Criteria" sub="Success metrics and verification methods" />
      {(() => {
        const accSec = sections.find(s => s.section_key === 'acceptance');
        const items = accSec?.compiled_data?.criteria || [];
        if (!items.length) return <em style={{ fontSize: '10pt', color: '#94A3B8' }}>No criteria defined.</em>;
        return (
          <PrintTable headers={[
            { label: 'ID' },
            ...
            {items.map((c, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                <td style={{ padding: '8pt', fontWeight: 700, width: '40pt' }}>{c.id || `AC-${i+1}`}</td>
                <td style={{ padding: '8pt' }}>
                  <div style={{ fontWeight: 600, fontSize: '10pt' }}>{c.description}</div>
                  {c.measurement && <div style={{ fontSize: '8.5pt', color: '#64748B', marginTop: '2pt' }}>Measurement: {c.measurement}</div>}
                </td>
                <td style={{ padding: '8pt', width: '100pt' }}>{c.verification || 'Manual Inspection'}</td>
                <td style={{ padding: '8pt', width: '80pt' }}>{c.responsible || 'Project Manager'}</td>
              </tr>
            ))}
          </PrintTable>
        );
      ...
      {/* --- 6. QUALITY FRAMEWORK --- */}
      <SectionTitle title="6. Quality Framework & Standards" sub="Compliance metrics and domain standards" />
      {(() => {
        const qualSec = sections.find(s => s.section_key === 'quality');
        const standards = qualSec?.compiled_data?.standards || [];
        const metrics = qualSec?.compiled_data?.metrics || [];
        return (
          <div style={{ pageBreakInside: 'avoid' }}>
            <div style={{ marginBottom: '16pt' }}>
              <div style={{ fontSize: '9pt', fontWeight: 700, color: '#475569', marginBottom: '8pt', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Applied Standards</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6pt' }}>
                {standards.map((s, i) => <span key={i} style={{ fontSize: '8pt', padding: '3pt 8pt', background: '#EFF6FF', color: '#1E40AF', borderRadius: '4pt', fontWeight: 600, border: '1px solid #DBEAFE' }}>{s}</span>)}
              </div>
            </div>
            <PrintTable headers={[
              { label: 'Metric' },
              ...
              {metrics.map((m, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  <td style={{ padding: '8pt', fontWeight: 700 }}>{m.metric}</td>
                  <td style={{ padding: '8pt' }}>{m.target}</td>
                  <td style={{ padding: '8pt' }}>{m.verification}</td>
                </tr>
              ))}
            </PrintTable>
          </div>
      ...
      {/* --- 7. DEPENDENCY MAP --- */}
      <SectionTitle title="7. Dependency Map" sub="Critical internal and external linkages" />
      {(() => {
        const depSec = sections.find(s => s.section_key === 'dependencies');
        const internal = depSec?.compiled_data?.internal || [];
        const external = depSec?.compiled_data?.external || [];
        return (
          <div>
            <div style={{ fontSize: '11pt', fontWeight: 700, marginBottom: '10pt', color: '#0F172A' }}>Internal Dependencies</div>
            <PrintTable headers={[{ label: 'Dependency' }, { label: 'Linked Task' }, { label: 'Owner' }]}>
              {internal.map((d, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  <td style={{ padding: '8pt', fontWeight: 600 }}>{d.name}</td>
                  <td style={{ padding: '8pt', color: '#5929d0' }}>{d.linked_task}</td>
                  <td style={{ padding: '8pt' }}>{d.owner}</td>
                </tr>
              ))}
            </PrintTable>

            <div style={{ fontSize: '11pt', fontWeight: 700, marginBottom: '10pt', color: '#0F172A', marginTop: '20pt' }}>External Dependencies</div>
            <PrintTable headers={[{ label: 'Dependency' }, { label: 'Linked Task' }, { label: 'Owner' }]}>
              {external.map((d, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  <td style={{ padding: '8pt', fontWeight: 600 }}>{d.name}</td>
      ...
      {/* --- 8. RISK REGISTER --- */}
      <SectionTitle title="8. Risk Register & Mitigation" sub="High and medium impact risks with mitigation plans" />
      <PrintTable headers={[
        { label: 'Ref' },
        { label: 'Risk Description' },
        { label: 'Sev', align: 'center' },
        { label: 'Mitigation Plan' },
        { label: 'Owner' }
      ]}>
        {risks.map((r, i) => (
          <tr key={i} style={{ borderBottom: '1px solid #F1F5F9', background: r.severity === 'High' ? '#FFF1F2' : 'transparent' }}>
            <td style={{ padding: '8pt', fontWeight: 700, width: '40pt' }}>{r.risk_ref}</td>
            <td style={{ padding: '8pt', fontSize: '8.5pt', lineHeight: 1.4 }}>{r.description}</td>
            <td style={{ padding: '8pt', textAlign: 'center', width: '40pt' }}>
              <span style={{ fontSize: '7.5pt', padding: '2pt 6pt', background: r.severity === 'High' ? '#BE123C' : r.severity === 'Medium' ? '#B45309' : '#15803D', color: '#fff', borderRadius: '4pt', fontWeight: 700 }}>
                {r.severity}
              </span>
            </td>
            <td style={{ padding: '8pt', fontSize: '8.5pt', color: '#475569', lineHeight: 1.4 }}>{r.mitigation}</td>
            <td style={{ padding: '8pt', fontSize: '8.5pt' }}>{r.owner || 'TBD'}</td>
          </tr>
        ))}
      </PrintTable>
    </div>
      ...
      {/* --- FOOTER --- */}
      <div style={{ marginTop: 60, borderTop: '1px solid #E2E8F0', paddingTop: 20, paddingBottom: 40, textAlign: 'center', fontSize: 9, color: '#94A3B8' }}>
        Proposal generated via Bid Management System (MEI-Aegis) · Version 1.0.0<br/>
        © {new Date().getFullYear()} Centific. All rights reserved.
      </div>
    </div>
  );
}
