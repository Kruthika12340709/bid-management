...
export function MiniArea({ data, color = '#5929d0', height = 60, fill = true, gradientId = 'ma' }) {
  const w = 100, h = 100;
  const max = Math.max(...data, 1);
  const pts = data.map((v, i) => [i * (w / (data.length - 1)), h - ((v / max)) * h * 0.85 - 5]);
...
export function BarPair({ labels, a, b, colorA = '#5929d0', colorB = '#CF008B', height = 200 }) {
...
export function Donut({ value, total = 100, color = '#5929d0', size = 140, label, sub }) {
...
export function Sparkline({ data, color = '#5929d0', height = 28 }) {
  const W = 80, H = 28;
  const max = Math.max(...data, 1);
